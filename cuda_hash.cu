...
// Calls changed for sorting hash list by virtual address
...
int binary_search_virt_addr_cpu(page_hash *active_hints, unsigned long virtual_address,uint32_t hash_value, int num_active_hints) 
{
	/*
		   Binary search hash within the old hash list.
		   Make sure it doesn't be the same virtual address.  
	 */
	int low=0,mid=0,high = num_active_hints-1;
	while(low<=high)
	{
		mid= (high+low)/2;
		if(active_hints[mid].vm_virt_address==virtual_address)
		{
			active_hints[mid].hash_value=hash_value;
			return 0;

		}	
		else if(active_hints[mid].vm_virt_address>virtual_address)
			high=mid-1;
...
//	printf("Unique Virtual Addresses = %d and Extended List size = %d\n",unique_virt_address_val,extended_list_size);
	...
	if(unique_virt_address_val>0)
	{
		...
		extend_list<<<num_blocks,512>>>(page_hash_extended,current_hints_gpu,is_virt_addr_present,is_virt_addr_present_prefix,num_active_hints,hints_generated);
	...
	//in the previous list but are present in the current
		free(*page_hash_old);
		*page_hash_old=(page_hash*)malloc(extended_list_size*sizeof(page_hash));
		cudaMemcpy(*page_hash_old,page_hash_extended,extended_list_size*sizeof(page_hash),cudaMemcpyDeviceToHost);
...
//if there are unique virt addr in current hints then page_hash_old will be active hints
	else 
	{
		...
		free(*page_hash_old);
		*page_hash_old=(page_hash*)malloc(num_active_hints*sizeof(page_hash));
		cudaMemcpy(*page_hash_old,active_hints_gpu,num_active_hints*sizeof(page_hash),cudaMemcpyDeviceToHost);
	}
...
int generate_hints(page_hash *page_hash_gpu,page_hash **hints_cpu,int *diff_map_gpu,int num) 
{
	page_hash *hints_gpu;	
	int *is_a_hint;
...
__global__ void get_active_hints(page_hash *page_hash_gpu_old,page_hash *active_hints_gpu,int *is_hashed_page_mapped_gpu,int *is_hashed_page_mapped_prefix_sum_gpu,int num_old) 
{
	int thread_id = blockIdx.x*blockDim.x+threadIdx.x;
	int rank=0;
...
unsigned int cuda_hash(char *page_data, page_hash *page_hash_cpu, unsigned long **hints,int page_size,int total_threads,page_hash **page_hash_old,int *are_old_hashes_available,unsigned int *num_old_hashes,unsigned long  *mapped_pages_list,unsigned long num_mapped_pages) {



	char *page_data_gpu;
	int size= total_threads*PAGE_SIZE;
	int num=total_threads;
	int extended_list_size=0;
	int num_active_hints=0;
	int i =0;
	int *diff_map_gpu=NULL;
	unsigned int num_blocks;
	unsigned int num_old_hashes_val = *num_old_hashes;
	unsigned int hints_generated =0;
	float cudata_transfer_time,cuhash_time;
	page_hash *page_hash_gpu;
//	page_hash *page_hash_cpu_temp;
	page_hash *page_hash_gpu_old;
	page_hash *active_hints_gpu;
	cudaEvent_t start,stop;	
	double begin,end,hash_time,sort_time,data_transfer_time;


	cudaMalloc((void**)&page_data_gpu,size*sizeof(char));
	cudaMalloc((void**)&page_hash_gpu,num*sizeof(page_hash));
	...
	cudaErrorCheck(cudaMemcpy(page_hash_gpu,page_hash_cpu,num*sizeof(page_hash),cudaMemcpyHostToDevice));
//The above copy is need as "page_hash_cpu" has "vm_virt_address" and "cuda_virt_address" fields aleardy filled
	
//	page_hash_cpu_temp = (page_hash*)malloc(num*sizeof(page_hash));
	
	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&cudata_transfer_time, start, stop);
	end = clock();
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
	data_transfer_time= (end-begin)/(CLOCKS_PER_SEC);
	tprint("\nTime taken to transfer page data for hash to GPU is %.10lf\n", data_transfer_time);
	tprint("\nTime taken to transfer page data for hash to GPU by cuEvent is(ms) %.10lf\n", cudata_transfer_time/1000);

/*
	for(i=0;i<num;i++) {
		dprint("\nHash before GPU Computation %d = %u \n ", i, page_hash[i]);
	}*/


	num_blocks=(num/THREADS_PER_BLOCK) + 1;
...
/*	
	This function computes the hash of pages on GPU and stores in page_hash_gpu

 	** page_hash_gpu initially sorted by virtual_address of pages
 ...
 */
	...
	cudaEventElapsedTime(&cuhash_time, start, stop);
	cudaEventDestroy(start);
	cudaEventDestroy(stop);
	hash_time = (end- begin)/(CLOCKS_PER_SEC);
	tprint("\nTime to hash the pages is %.10lf \n", hash_time);
	tprint("\nTime to hash the pages by cuevent is %.10lf \n", cuhash_time/1000);*/
	//	cudaMemcpy(page_hash,page_hash_gpu,num*sizeof(uint32_t),cudaMemcpyDeviceToHost);
	//result=cudaMemcpy(page_data,page_data_gpu,size*sizeof(char),cudaMemcpyDeviceToHost);

	cudaFree(page_data_gpu);
//	printf("\n Hashes Calculated \n");
	
//	cudaMemcpy(page_hash_cpu_temp,page_hash_gpu,num*sizeof(page_hash),cudaMemcpyDeviceToHost);
	
	//Code to merge the old and new hashes 

	/*
	 * Intially we have a unsorted hash list
	 * store this in page_hash_old variable for the first time
	 * In next iteration we will have another unsorted hash list.
	 * we want to merge these two list

	 */

	//if this is the first pass then old hashes are not available
	//no need to extend list


	if(*are_old_hashes_available ==0) {
		//printf("First pass. Copying old hashes \n");
		
//		cudaMemcpy(page_hash_cpu_temp,page_hash_gpu,num*sizeof(page_hash),cudaMemcpyDeviceToHost);

//Begin: Code to generate hints on GPU :

//		page_hash *hints_cpu;		

		page_hash *temp_page_hash_old;
		unsigned long *temp_hints;
		cuda_sort(page_hash_gpu,total_threads);
//		hints_generated=generate_hints(page_hash_gpu,&hints_cpu,NULL,num);
		hints_generated=generate_hints(page_hash_gpu,page_hash_old,NULL,num);
		*num_old_hashes = hints_generated;
		if(hints_generated>0)
		{
			*hints=(unsigned long*)malloc(hints_generated*sizeof(unsigned long));
			temp_hints=*hints;
			temp_page_hash_old=*page_hash_old;
			for(i=0;i<hints_generated;i++)
				temp_hints[i]=temp_page_hash_old[i].vm_virt_address;
			*are_old_hashes_available=1;
		}
//		free(hints_cpu);

//End: Code to generate 
	}

	else if(*are_old_hashes_available==1 && num_old_hashes_val>0 && num > 0) 
	{
	
	//	printf("Second pass trying to extend list\n");

//Begin: Code to find the active hints which are still mapped 
		...
		cudaMalloc((void**)&is_hashed_page_mapped_gpu,num_old_hashes_val*sizeof(int));
		cudaMalloc((void**)&is_hashed_page_mapped_prefix_sum_gpu,num_old_hashes_val*sizeof(int));

		cudaMalloc(&page_hash_gpu_old,num_old_hashes_val*sizeof(page_hash));	
		cudaMemcpy(page_hash_gpu_old,*page_hash_old,num_old_hashes_val*sizeof(page_hash),cudaMemcpyHostToDevice);


		if(num_mapped_pages>0)	
			num_active_hints=find_active_hints(page_hash_gpu_old,mapped_pages_list,is_hashed_page_mapped_gpu,is_hashed_page_mapped_prefix_sum_gpu,num_old_hashes_val,num_mapped_pages);		

		if(num_active_hints>0) 
		{
			cudaMalloc((void**)&active_hints_gpu,num_active_hints*sizeof(page_hash));
			num_blocks=ceil((double)num_old_hashes_val/(double)512);
			get_active_hints<<<num_blocks,THREADS_PER_BLOCK>>>(page_hash_gpu_old,active_hints_gpu,is_hashed_page_mapped_gpu,is_hashed_page_mapped_prefix_sum_gpu,num_old_hashes_val);
...
//End: Code to find the active hints

	}
	
	// This function sorts the page hashes.

//	cudaMemcpy(page_hash_gpu,page_hash_cpu_temp,num*sizeof(page_hash),cudaMemcpyHostToDevice);
...
//	cudaMemcpy(page_hash_cpu_temp,page_hash_gpu,num*sizeof(page_hash),cudaMemcpyDeviceToHost);
	...
	cudaErrorCheck(cudaMemcpy(page_hash_cpu,page_hash_gpu,num*sizeof(page_hash),cudaMemcpyDeviceToHost));
...
/*
	...
	cudaEventDestroy(stop);
	data_transfer_time = (end - begin)/(CLOCKS_PER_SEC);
	tprint("\n Time to transfer page hash from GPU to CPU is %.10lf\n",data_transfer_time);
	tprint("\n Time to transfer page hash from GPU to CPU by cuevent is %.10lf\n",cudata_transfer_time/1000);*/

// Computing the difference of old hash and new hash	

	if(*are_old_hashes_available==1 && num>0 && num_old_hashes_val>0) 
	{

//Compute diff only when there are active hints

//Begin: Code to Compute Diff Map

		if(num_active_hints>0)
		{	
			cudaErrorCheck(cudaMalloc(&diff_map_gpu,num*sizeof(int)));

	//Assuming page_hash_old_gpu already present in memory

		cuda_sort(active_hints_gpu,num_active_hints);
	//	printf("Cuda sort within diff map code done \n");
		num_blocks= ceil((double)num/(double)512);	
	
		diff_hash<<<num_blocks,512>>>(active_hints_gpu,page_hash_gpu,num,diff_map_gpu,num_active_hints);
		

		}

//End: Code to Compute Diff Map

//Begin: Code to generate hints on GPU for Rounds>=2
		...
		page_hash *current_hints_gpu=NULL;
	
		hints_generated=generate_hints(page_hash_gpu,&hints_cpu,diff_map_gpu,num);
		if(hints_generated>0)
		{
			...
			*hints=(unsigned long*)malloc(hints_generated*sizeof(unsigned long));
			temp_hints = *hints;
			for(i=0;i<hints_generated;i++)
				temp_hints[i]=hints_cpu[i].vm_virt_address;

			cudaErrorCheck(cudaMalloc((void**)&current_hints_gpu,hints_generated*sizeof(page_hash)));
			cudaErrorCheck(cudaMemcpy(current_hints_gpu,hints_cpu,hints_generated*sizeof(page_hash),cudaMemcpyHostToDevice));

			free(hints_cpu);
		}


//End: Code to generate hints on GPU


/*Now you have new hints and active hints: 
  Generate the consolidated hint history
  to be used in next round

  */

/*Begin: Code to Extend list and Consildate previous round's
         Active hints and Current Round hints

  */
		extended_list_size=0;

		if(num_active_hints>0 && hints_generated>0)
			extended_list_size=cuda_extend_hash_list(page_hash_old,current_hints_gpu,active_hints_gpu,num_active_hints,hints_generated);
	
		else if(num_active_hints !=0 && hints_generated==0)
		{
		free(*page_hash_old);
		*page_hash_old=(page_hash*)malloc(num_active_hints*sizeof(page_hash));
		cudaMemcpy(*page_hash_old,active_hints_gpu,num_active_hints*sizeof(page_hash),cudaMemcpyDeviceToHost);
		...
		{
		free(*page_hash_old);
		*page_hash_old=(page_hash*)malloc(hints_generated*sizeof(page_hash));
		cudaMemcpy(*page_hash_old,current_hints_gpu,hints_generated*sizeof(page_hash),cudaMemcpyDeviceToHost);
		extended_list_size=hints_generated;
		}
		
	//	printf("Num old hashes value = %d and Extended List size = %d Active Hints = %d\n",num_old_hashes_val,extended_list_size,num_active_hints);	
		*num_old_hashes=extended_list_size; //new hashes needs to be calculated

		if(extended_list_size>0)
			*are_old_hashes_available=1;
		else
			*are_old_hashes_available=0;
		
		if(num_active_hints>0)
			cudaErrorCheck(cudaFree(active_hints_gpu));
		if(hints_generated>0)
			cudaErrorCheck(cudaFree(current_hints_gpu));

	}
	
	cudaFree(page_hash_gpu);
//	free(page_hash_cpu_temp);
	//diff map is needed for generating final hints
	//by this time we must have completed first pass of computing hashes

//End: Code to Extend list
	return hints_generated;
